...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:PoseRefinePairH5Dataset=None):
  ...
  for b in range(0,len(poseA),bs):
    ...
    if isinstance(glctx, dr.RasterizeCudaContext):
      ...
      rgb_r, depth_r, normal_r = moderngl_render(K=K, H=H, W=W, ob_in_cams=poseA[b:b+bs], get_normal=cfg['use_normal'], glctx=glctx, mesh_tensors=mesh_tensors, output_size=cfg['input_resize'], bbox2d=bbox2d_ori[b:b+bs], use_light=True, extra=extra)
    rgb_rs.append(rgb_r)
